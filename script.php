<?php 
	...
	if(isset($_POST['submit'])){
		...
		if(filesize("db.json") == 0){
			...
			$first_record = array("contantInfo"=>array($new_message));
			...
			array_push($old_records->contantInfo, $new_message);
		...
		if(!file_put_contents("db.json", json_encode($data_to_save, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE), LOCK_EX)){
